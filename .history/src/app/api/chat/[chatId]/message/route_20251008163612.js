    ...
    const result = streamText({
      ...
      tools: {
        ...
        // url_context: google.tools.urlContext({}),
          google_search: tool({
        description: "Search Google and return short web results",
        inputSchema: z.object({ query: z.string() }),
        execute: async ({ query }) => {
          // perform the actual search manually
          const res = await fetch(`https://www.googleapis.com/customsearch/v1?key=${process.env.GOOGLE_API_KEY}&cx=${process.env.GOOGLE_CX}&q=${encodeURIComponent(query)}`);
          const data = await res.json();
          return data.items?.map(i => `${i.title}: ${i.link}`).join("\n") || "No results found.";
        },
      }),
