                      ...
                      switch (part.type) {
                        case "text":
                          return (
                            ...
                            >
                               <article className="prose prose-lg dark:prose-invert max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 md:p-10">
        <ReactMarkdown
          children={markdownFromApi}
          remarkPlugins={[remarkGfm]}
          components={{
            // Use the same override for code blocks to enable syntax highlighting
            code({ node, inline, className, children, ...props }) {
              const match = /language-(\w+)/.exec(className || '');
              return !inline && match ? (
                <div className="my-4 rounded-md overflow-hidden"> {/* Added a wrapper for margin and rounded corners */}
                  <SyntaxHighlighter
                    style={vscDarkPlus}
                    language={match[1]}
                    PreTag="div"
                ...
                <code className="font-mono text-sm bg-gray-200 dark:bg-gray-700 rounded-md px-1.5 py-0.5" {...props}>
                  {children}
                </code>
              );
            },
