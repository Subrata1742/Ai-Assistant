...
export default function ChatPage() {
  ...
  //   // 1. Fetch the current list of chats directly from the server.
  //   const currentChats = await fetchChats();

  //   // 2. Find all chats that are untitled (and thus likely empty).
  ...
  const handleNewChat = useCallback(async () => {
    // ✅ This line is the fix. We add `|| []` as a fallback.
    const currentChats = (await fetchChats()) || [];
