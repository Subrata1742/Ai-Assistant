                ...
                >
                  <div className="text-slate-400">
                    ...
                    {message.parts.map((part, index) => {
                     ...
                     {message.role === "user" ? "" : <Button onClick={() => handleSpeak(message)} className="bg-transparent border text-slate-500"> <Volume2/></Button>}
                  </div>
                </div>
              ))}
            ...
            <form onSubmit={handleSubmit}>
              <div className="flex items-center space-x-2 border-t border-gray-200 bg-gray-50 p-4">
                ...
                <Button
                  type="button"
                  onClick={handleMic}
                  className={`rounded-full p-2 ${isListening ? 'bg-red-500 hover:bg-red-600 text-white' : ''}`}
                  disabled={status !== 'ready'}
                  title={isListening ? 'Stop listening' : 'Listen with microphone'}
                >
                  <Mic />
                </Button>
                {status === "submitted" || status === "streaming" ? (
