                          ...
                          return (
                            ...
                            >
                              <ReactMarkdown
                                ...
                                components={{
                                  p: ({ node, children }) => {
    // A helper function to recursively check for a non-inline code block
    const hasCodeBlock = (elements) => {
      if (!Array.isArray(elements)) return false;

      return elements.some(el => {
        // Check if the element itself is a code block
        if (el?.type === 'code' && !el?.props?.inline) {
          return true;
        }
        // Recursively check its children
        if (el?.props?.children) {
          return hasCodeBlock(el.props.children);
        }
        return false;
      });
    };

    // If a code block is found anywhere inside, don't wrap with <p>
