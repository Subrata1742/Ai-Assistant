 ...
 const [files, setFiles] = useState(undefined);
  ...
  //   // Speak AI reply with SpeechSynthesis API
    const speak = (text) => {
      ...
      let cleanText = text.replace(/```[\s\S]*?```/g, "Here you can see code example");
              ...
              {messages.map((message) => (
                ...
                default:
                    return null;
                      ...
                      {message.role === "user" ? "" : <Button onClick={speak(part.text)} className="bg-transparent border text-slate-500"> <Volume2/></Button>}
                    })}
