...
export default function ChatSidebar({
  ...
  const DisableNewChat= ()=>{
   const emptyChats = chats.filter((chat) => chat.id===activeChatId);
   if(!emptyChats.title){
return true;
   }
  }
      ...
      <SidebarContent >
        <SidebarGroup className="pt-0">
          <div className="p-4  z-10 border-b sticky top-0 bg-gray-50">
            <Button  onClick={onNewChat}  className="mb-2 w-full" disabled={chats.filter((chat)=>chat.id===activeChatId && !chat.title)}>
              + New Chat
            </Button>
          </div>
          <SidebarGroupContent>
