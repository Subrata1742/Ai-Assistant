...
export default function ChatSidebar({
  ...
  return (
    ...
    //   </ul>
    // </div>
    <Sidebar className="mt-18 overflow-y-auto scrollbar-hide">
      <SidebarContent>
        <SidebarGroup>
          <div className="p-4 pt-6 border-b w-full sticky top-0  bg-amber-600"><Button onClick={onNewChat} className="mb-2 w-full">
          + New Chat
       </Button></div>
          <SidebarGroupContent>
            <SidebarMenu>
              {chats.map((c) => (
                <SidebarMenuItem key={c.id} className="flex my-1 justify-between w-full">
                  <SidebarMenuButton
                    variant={c.id === activeChatId ? "secondary" : "ghost"}
                    onClick={() => onSelectChat(c.id)}
                    asChild
