...
export default function ChatSidebar({
  ...
  const showChat =chats.filter((chat) => chat.title);
    ...
    <Sidebar className=" pt-0">
      <SidebarContent >
        <SidebarGroup className="pt-0">
          <div className="p-4  z-10 border-b-2 sticky top-0 bg-gray-50">
            <Button  onClick={onNewChat}  className="mb-2 w-full disabled:bg-gray-950" disabled={currentChat[0]?.title===""}>
              <IoCreate/> New Chat
            </Button>
          </div>
          <SidebarGroupContent>
            <SidebarMenu className="overflow-auto">
              {showChat.map((c) => (
                ...
                >
                  <SidebarMenuButton
                    variant={c.id === activeChatId ? "outline" : "default"}
                    onClick={() => onSelectChat(c.id)}
                    asChild
                    className={c.id === activeChatId ? " hover:bg-gray-800 hover:text-white" : " hover:bg-gray-200"  }
                  >
                    <div className="w-full px-2 overflow-hidden text-ellipsis whitespace-nowrap ">
                      {c.title ? c.title : "New"}
                    </div>
                  </SidebarMenuButton>
                  <Button
